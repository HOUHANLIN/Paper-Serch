...
- `app/server.py`：Flask 应用与路由（首页检索、自动工作流、下载等）。
- `app/web/`：Web 层逻辑（表单解析、SSE 搜索封装）。
- `app/core/`：通用服务（方向拆解、检索式生成、AI 摘要、BibTeX 组装等）。
- `app/sources/`：文献数据源（当前默认 PubMed）。
- `app/ai/`：AI Provider（OpenAI/Gemini）。
...
### AI 摘要并发
- `app/core/ai_summary.py` 会对每篇文章单独调用一次 summarize，并发数默认为“每篇文章一个并发 worker”（不限制）。
